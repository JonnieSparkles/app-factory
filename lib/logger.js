...
class Logger {
  ...
  showBalance(balance) {
    // Debug: Log the balance object structure
    console.log('DEBUG: Balance object:', JSON.stringify(balance, null, 2));
    ...
    if (typeof balance === 'object' && balance !== null) {
      ...
      if (balance.receivedApprovals && balance.receivedApprovals.length > 0) {
        const totalSharedCredits = balance.receivedApprovals.reduce((sum, approval) => sum + (approval.amount || 0), 0);
        this.info(`💰 Wallet balance: ${walletBalance} winc | Shared credits: ${totalSharedCredits} winc`);
      } else {
        this.info(`💰 Wallet balance: ${walletBalance} winc | No shared credits available`);
      }
    } else {
      walletBalance = balance || '0';
      this.info(`💰 Wallet balance: ${walletBalance} winc`);
    }
  }
