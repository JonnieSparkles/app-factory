...
class Logger {
  ...
  showDeploymentSummary(stats) {
    ...
    const estimatedTotalSize = totalFiles > 0 && changedFiles > 0 ? Math.round((uploadedSize / changedFiles) * totalFiles) : uploadedSize;
    const savedSize = Math.max(0, estimatedTotalSize - uploadedSize);
