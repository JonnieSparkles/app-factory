...
export async function logDeploymentResult(result) {
  logger.newLine();
  logger.success('🎉 Deployment complete!');
  logger.info(`   File: ${result.filePath}`);
  logger.info(`   Commit: ${result.commitHash}`);
  logger.info(`   TX ID: ${result.txId}`);
  logger.info(`   ArNS: ${result.undername} (expires in ${Math.floor(result.ttl / 86400)} days)`);
  logger.info(`   Size: ${formatBytes(result.fileSize)}`);
  logger.info(`   Duration: ${formatDuration(result.duration)}`);
