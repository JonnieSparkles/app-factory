...
export async function logDeploymentResult(result, deploymentStats = null) {
  ...
  // Show deployment summary table at the end if we have stats and it was successful
  if (result.success && deploymentStats) {
    logger.newLine();
    logger.showDeploymentSummary({
      changedFiles: deploymentStats.changedFiles || 0,
      unchangedFiles: deploymentStats.unchangedFiles || 0,
      totalSize: formatBytes(deploymentStats.totalSize || 0),
      manifestTxId: result.txId || result.manifestTxId || 'N/A',
