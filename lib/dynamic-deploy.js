...
export class DynamicDeployer {
  ...
  async deploy(testMode = false) {
    ...
    try {
      ...
      logger.startSpinner(`📤 Uploading ${changedFiles.length} changed files...`);
      const { newFileIds, fileHashes } = await this.uploadChangedFiles(changedFiles, testMode);
      logger.succeedSpinner(`Uploaded ${changedFiles.length} files successfully`);
      ...
      logger.startSpinner(`🔗 Creating ArNS record: ${commitInfo.shortHash} → ${manifestTxId}`);
      const undername = await this.createArNSRecord(commitInfo.shortHash, manifestTxId, testMode);
      logger.succeedSpinner(`ArNS record created: ${undername}`);
      ...
      // Show deployment summary with boxed table
  ...
  async createArNSRecord(commitHash, manifestTxId, testMode = false) {
    try {
      if (testMode) {
        logger.info(`🧪 Test mode: Would create ArNS record ${commitHash} → ${manifestTxId}`);
        return commitHash;
      }
      ...
      if (existingRecord) {
        logger.warning(`⚠️ ArNS record already exists for ${commitHash}`);
        return commitHash;
      }

      // Create new ArNS record
      logger.showArNSOperation('Creating', commitHash, manifestTxId);
      logger.info(`🔍 Using TTL: 60 seconds`);
      const result = await createUndernameRecord(ant, commitHash, manifestTxId, 60);
      if (result.success) {
        logger.success(`ArNS record created: ${result.undername || commitHash}`);
        return commitHash;
      } else {
