...
async function formatDeploymentMessage(deploymentData) {
  ...
  try {
    ...
    return {
      ...
      embed: null  // No embed needed when template works
    };
  } catch (error) {
...
export async function sendDiscordNotification(deploymentData, forceAnnounce = false) {
  try {
    ...
    // Use the template content as the main message, with embed as fallback only
    let payload;
    
    if (fallbackEmbed) {
      // Use fallback embed if template failed
      payload = {
        content: content,
        embeds: [fallbackEmbed]
      };
    } else {
      // Use template content as the main message (no embed needed)
      payload = {
        content: content
      };
    }
