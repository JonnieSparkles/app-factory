...
export async function uploadToArweave(mediaBuffer, contentType = 'application/octet-stream', appName = null, jwk = null, customTags = []) {
  try {
    logger.startSpinner(`☁️ Uploading ${(mediaBuffer.length / 1024).toFixed(1)}KB to Arweave...`);
    ...
    logger.succeedSpinner(`Uploaded: ${uploadResult.id} (${uploadResult.winc} winc)`);
    
    return uploadResult.id;
  } catch (error) {
    logger.failSpinner(`Arweave upload failed: ${error.message}`);
    throw error;
  }
}
