...
export async function uploadToArweave(mediaBuffer, contentType = 'application/octet-stream', appName = null, jwk = null, customTags = []) {
  try {
    ...
    if (config.useSharedCredits) {
      // Let SDK automatically find and use available credit share approvals
      // SDK will use approvals closest to expiration, then lowest amounts, then signer's balance
      console.log(`🔗 Using shared credits (SDK will automatically find available approvals)`);
      // Don't set paidBy - let the SDK handle it automatically
    }
...
export async function uploadManifest(pathMap, indexPath = 'index.html', appName = null, jwk = null) {
  try {
    ...
    if (config.useSharedCredits) {
      // Let SDK automatically find and use available credit share approvals
      console.log(`🔗 Using shared credits for manifest (SDK will automatically find available approvals)`);
      // Don't set paidBy - let the SDK handle it automatically
    }
