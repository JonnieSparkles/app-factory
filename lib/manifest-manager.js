...
export class ManifestManager {
  ...
  async loadManualOverrides() {
    ...
    try {
      if (await fileExists(overridesPath)) {
        ...
        console.log(`📋 Loaded ${Object.keys(overrides).length} manual TXID override(s)`);
        return overrides;
      }
    } catch (error) {
      console.warn(`⚠️ Could not load manifest-overrides.json: ${error.message}`);
    }
    return {};
  }
  ...
  async discoverAppFiles() {
    ...
    for (const file of allFiles) {
      ...
      // All files are deployable (getAllFilesInDirectory already excludes directories)
  ...
  async updateDeploymentTracker(commitHash, manifestTxId, changedFiles, fileHashes = {}, currentFiles = []) {
    ...
    const relativeChangedFiles = changedFiles.map(file => path.relative(this.appPath, file));
    ...
    if (currentFiles.length > 0) {
      const currentRelativePaths = new Set(
        currentFiles.map(file => path.relative(this.appPath, file))
      );
