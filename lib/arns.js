...
export async function getUndernameRecord(ant, undername) {
  try {
    ...
    if (error.message && error.message.includes('Unexpected token')) {
      console.log(`⚠️ ArNS service appears to be returning HTML instead of JSON. This may be a temporary service issue.`);
      console.log(`📝 Deployment will continue without ArNS record check.`);
    }
