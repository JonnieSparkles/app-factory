...
export class GitTracker {
  ...
  async validateGitState() {
    try {
      ...
      const gitDir = await git.findRoot({ fs, dir: process.cwd() });
      if (!gitDir) {
        throw new Error('Not in a git repository');
      }

      // Check for uncommitted changes that might conflict
      const status = await git.status({ fs, dir: this.dir });
      const hasUncommittedChanges = Object.values(status).some(files => files.length > 0);
      
      if (hasUncommittedChanges) {
        logger.warning('⚠️ Uncommitted changes detected - proceeding with caution');
      }
  ...
  async isGitRepository() {
    try {
      await git.findRoot({ fs, dir: process.cwd() });
      return true;
    } catch (error) {
      return false;
    }
