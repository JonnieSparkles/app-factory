  ...
  async getChangedFilesSinceCommit(commitHash, useHashing = false) {
    try {
      ...
      try {
        ...
        // Commit not found, fall back to hash-based detection
        console.log(`⚠️ Commit ${commitHash} not found in git history`);
        console.log(`🔄 Falling back to file hash-based change detection...`);
        return await this.getChangedFilesByHashing(commitHash);
      }
