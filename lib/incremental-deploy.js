...
import { readFile, formatBytes } from './utils.js';
...
export class IncrementalDeployer {
  ...
  async deploy(testMode = false, useHashing = false) {
    ...
    try {
      ...
      // 3. Get last deployment commit
      const lastDeployCommit = await this.gitTracker.findLastDeployCommit();
      console.log(`🔍 Last deployment commit: ${lastDeployCommit || 'none (first deployment)'}`);

      // 4. Find changed files
      const changedFiles = await this.gitTracker.getChangedFilesSinceLastDeploy(lastDeployCommit, useHashing);
      console.log(`📁 Changed files: ${changedFiles.length}`);
      
      if (useHashing) {
        console.log(`🔍 Using file hashing for change detection`);
      } else {
        console.log(`🔍 Using git-based change detection`);
      }
      ...
      // 5. Load current manifest and tracker
      ...
      // 7. Update manifest with new file IDs
      const updatedManifest = await this.updateManifestWithNewFiles(currentManifest, newFileIds, changedFiles);
  ...
  async uploadChangedFiles(changedFiles, testMode = false) {
    ...
    for (let i = 0; i < changedFiles.length; i++) {
      ...
      try {
        ...
        console.log(`☁️ Uploading ${formatBytes(fileSize)} to Arweave...`);
        ...
        if (testMode) {
          ...
          const txId = await uploadToArweave(fileContent, wallet);
  ...
  async uploadManifest(manifest, testMode = false) {
    ...
    return await uploadManifest(manifestContent, wallet);
  }

  // ---------- Manifest Management ----------

  async updateManifestWithNewFiles(currentManifest, newFileIds, changedFiles) {
    const updatedManifest = { ...currentManifest };
    
    // Update version
    updatedManifest.version = this.manifestManager.incrementVersion(currentManifest.version);
    
    // Clean up existing manifest - remove tracking files
    const cleanedPaths = {};
  ...
  async createArNSRecord(commitHash, manifestTxId, testMode = false) {
    try {
      ...
      const existingRecord = await getUndernameRecord(commitHash);
      if (existingRecord) {
      ...
      const undername = await createUndernameRecord(commitHash, manifestTxId);
      return undername;
    } catch (error) {
