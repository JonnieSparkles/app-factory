...
export class AppFactory {
  ...
  async isMultiFileApp(appDir) {
    ...
    // But exclude shared directories like 'deploy' where multiple independent apps might live
    try {
      if (!(await isDirectory(appDir))) {
        return false;
      }

      // If the app directory is 'deploy', treat it as single-file
      // since multiple independent apps share that directory
      if (appDir === 'deploy') {
        return false;
      }
  ...
  async deployMultiFileApp(appId, app, appDir, options, startTime) {
    try {
      ...
      console.log(`📝 Creating ArNS record: ${shortHash} → ${manifestTxId}`);
