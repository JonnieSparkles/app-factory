...
Deploy your existing build output without restructuring your project:
...
1. **Copy deployment files**
   ...
   cp deploy.js your-project/
   cp package.json your-project/  # Or add dependencies to your existing package.json
   ```
...
3. **Configure environment**
   ```bash
   cp env.example .env
   # Edit .env with your ArNS/wallet details
   ```

4. **Deploy your build folder**
   ```bash
   node deploy.js --file dist/     # or build/, public/, etc.
   node deploy.js --file build/
   node deploy.js --file public/
   ```
...
│   ├── bundle.js
│   ├── styles.css
│   ├── manifest.json              # Auto-generated by deployment
│   └── deployment-tracker.json    # Auto-generated by deployment
├── lib/                   # Copied deployment modules
├── deploy.js              # Copied deployment script
...
```bash
npm run build                    # Your existing build process
node deploy.js --file dist/      # Deploy to Arweave
```

The system deploys from any directory you specify - no need to rename folders or change your project structure.
