...
1. **Remote Agent Integration** - Compatible with cursor/automerge workflows for autonomous development
...
3. Deploy:
   ...
   node deploy.js --file path/to/app/               # Directory (incremental)
   ```
...
Auto-Merge (validates & merges)
    ↓
Deploy (incremental: only changed files)
    ↓
...
- **Auto-Merge Workflow**: Validates and merges agent PRs automatically
...
```
Automatically uses incremental deployment:
...
--no-incremental      # Force full directory upload
--logs                # View deployment history
--stats               # View deployment statistics
```

## Incremental Deployment Details
...
See [Technical Documentation](./docs/INCREMENTAL_ARWEAVE_DEPLOYMENT.md) for implementation details.
